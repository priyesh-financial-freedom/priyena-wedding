...
export default function BudgetCategoryActions({ category }: Props) {
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      `Delete "${category.name}"? This is allowed only when no budget items are linked to the category.`,
    );
    ...
    if ((count ?? 0) > 0) {
      window.alert(
        `Cannot delete "${category.name}". ${count} ${
          count === 1 ? "budget item is" : "budget items are"
        } linked to this category. Move or delete those items first.`,
      );
      setDeleting(false);
      return;
    }
    ...
    if (deleteError) {
      window.alert(`Unable to delete category: ${deleteError.message}`);
      setDeleting(false);
      return;
    }
            ...
            <div className="flex items-center justify-between border-b px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Rename this category throughout the budget.
                </p>
              </div>
